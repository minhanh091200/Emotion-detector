...
# Main model to train the data
class MainMultiLayerFCNet(nn.Module):
...
if __name__ == '__main__':
    ...
    model = MainMultiLayerFCNet(output_size)
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            if ACC <= bestAccuracy:
                ...
                bestAccuracy = ACC
                torch.save(model.state_dict(), os.path.join(save_dir, 'Main.pth'))

    # After training loop
    loadedModel = MainMultiLayerFCNet(output_size)
    loadedModel.load_state_dict(torch.load(os.path.join(save_dir, 'Main.pth')))
